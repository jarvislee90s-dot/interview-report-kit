...
- **①′①″ 多源输入**：腾讯会议录制分享（公开链接免登录，顺带抓 AI 纪要参考）、本地纪要文档 md/txt/word/pdf（提取+agent 结构化）、任意转写网页（浏览器复制同通道）——统一收口 `minutes_raw.json`
- **③′ 录音分离转录**：FunASR（paraformer-zh + cam++）一次产出转写+说话人标签+时间戳，对话式标记落名；CPU 即可（RTF ≈ 0.3）；仅需快速总结时走 `asr.py` 单产物快速线
