...
- **`t` 允许缺省**：无时间戳来源（纯文本文档等）。`build_transcript.py` 对无 `t` 段落输出不带时间戳的发言块；`render.py` 的纪要版式（`ENTRY_RE`）与 `apply_corrections.py` 的段落数守卫相应把时间戳改为可选（无时间戳来源走 ④ 校正时守卫同样有效）。已验证空时间戳在现有模板下安全降级；timeline 模板标注"不建议无时间戳来源"。
